...
const configureLinterIfNecessary = ({ filename, prettierConfig }) => {
  fileExists(filename).then(exists => {
    if (exists) {
      return Promise.all([
        installDependency(prettierConfig),
        addExtensionToLintConfig(filename, prettierConfig)
      ]).catch(err => {
...
const configureLinters = () => linters.reduce(
  (promise, linter) => promise.then(() => configureLinterIfNecessary(linter)),
  Promise.resolve()
);
